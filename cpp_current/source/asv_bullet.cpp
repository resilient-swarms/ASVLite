...
void ASVLite::Asv::set_mass() {
    ...
    dynamics.M(0, 0) = asv_mass;
    dynamics.M(1, 1) = asv_mass;
    dynamics.M(2, 2) = asv_mass;
    dynamics.M(3, 3) = I_roll;
    dynamics.M(4, 4) = I_pitch;
    dynamics.M(5, 5) = I_yaw;
}
...
void ASVLite::Asv::set_damping_coefficient() {
    ...
    const double C_roll  = C_heave;
    const double C_pitch = C_heave;
    const double C_yaw   = C_heave;
...
void ASVLite::Asv::set_stiffness() {
    ...
    // Assuming rectangle shape for the water plane area.
    const double A = spec.L_wl * spec.B_wl;
    const double I_xx = spec.B_wl * pow(spec.L_wl, 3) / 12;
    const double I_yy = spec.L_wl * pow(spec.B_wl, 3) / 12;
    
    // Heave stiffness
    const double K_heave = A * Constants::SEA_WATER_DENSITY * Constants::G;
...
void ASVLite::Asv::set_wave_force() {
    ...
    const double A_waterplane = M_PI * a * b;
    ...
    if(dynamics.submersion_depth >= 0.0) {
        ...
        for(const RegularWave& wave : sea_surface->component_waves) {
            ...
            const Eigen::Matrix3d R (Eigen::AngleAxisd(dynamics.attitude.keys.z, Eigen::Vector3d::UnitX())*  // yaw  
                                    Eigen::AngleAxisd(dynamics.attitude.keys.y, Eigen::Vector3d::UnitY())*  // pitch 
                                    Eigen::AngleAxisd(dynamics.attitude.keys.x, Eigen::Vector3d::UnitZ())); // roll 
            ...
            // Lever
            const double lever_trans = b / 2;
            const double lever_long  = a / 2;
            // Set the wave pressue force matrix
            // const double scale = ((encountered_wave.height > spec.D)? spec.D/encountered_wave.height : 1) / sea_surface->component_waves.size();
...
void ASVLite::Asv::set_restoring_force() {
    ...
    if(dynamics.submersion_depth >= 0.0) {
        ...
        const Eigen::Vector3d elongation {std::clamp(dynamics.submersion_depth, spec.T - spec.D, 0.0), dynamics.attitude.keys.x, dynamics.attitude.keys.y};
...
void ASVLite::Asv::set_pose() {
    // First set attitude
    dynamics.attitude.keys.z += dynamics.X(5); // yaw
    dynamics.attitude.keys.z = Geometry::normalise_angle_PI(dynamics.attitude.keys.z);
    dynamics.attitude.keys.x += dynamics.X(3);
    dynamics.attitude.keys.y += dynamics.X(4);
...
ASVLite::Asv::Asv(const AsvSpecification& spec, const SeaSurface* sea_surface, const Geometry::Coordinates3D& position, const Geometry::Coordinates3D& attitude) :
    ...
    // Create required Bullet components
    btDefaultCollisionConfiguration* collisionConfiguration = new btDefaultCollisionConfiguration();
    btCollisionDispatcher* dispatcher = new btCollisionDispatcher(collisionConfiguration);
    btBroadphaseInterface* broadphase = new btDbvtBroadphase(); // Broadphase is still needed for efficiency
    btSequentialImpulseConstraintSolver* solver = new btSequentialImpulseConstraintSolver();

    // Create Bullet Physics world
    bullet_world = new btDiscreteDynamicsWorld(dispatcher, broadphase, solver, collisionConfiguration);

    // Initialise Bullet Physics world (without collision detection)
    const btVector3 gravity(0, -Constants::G, 0);
    bullet_world->setGravity(gravity);

    // Create boat rigid body
    btTransform boat_transform;
    boat_transform.setIdentity();
    boat_transform.setOrigin(btVector3(dynamics.position.keys.y, dynamics.position.keys.z, dynamics.position.keys.x)); // Start above water
    btScalar boat_mass = dynamics.M(2,2); 
    btVector3 inertia(dynamics.M(3,3), dynamics.M(4,4), dynamics.M(5,5));
    btCollisionShape* boat_shape = new btBoxShape(btVector3(spec.B_wl, spec.D, spec.L_wl)); // Approximate boat size

    btMotionState* boat_motion_state = new btDefaultMotionState(boat_transform);
    btRigidBody::btRigidBodyConstructionInfo boat_info(boat_mass, boat_motion_state, boat_shape, inertia);
    bullet_boat = new btRigidBody(boat_info);
    bullet_boat->setCollisionFlags(btCollisionObject::CF_NO_CONTACT_RESPONSE); // disable collision response
    bullet_world->addRigidBody(bullet_boat);
}
...
void ASVLite::Asv::step_simulation(const Geometry::Coordinates3D& thrust_position, const Geometry::Coordinates3D& thrust_magnitude) {
    ...
    set_restoring_force();
    set_net_force();
    bullet_boat->activate();
    bullet_boat->applyCentralForce(btVector3(0.0, dynamics.M(2,2) * Constants::G, 0.0)); // still water buoyance
    bullet_boat->applyCentralForce(btVector3(dynamics.F(1), dynamics.F(2), dynamics.F(0)));
    bullet_boat->applyTorque(btVector3(dynamics.F(3), dynamics.F(5), dynamics.F(4)));
    bullet_world->stepSimulation(dynamics.time_step_size/1000.0, dynamics.max_substeps);
    // Get updated boat state
    btTransform boat_transform;
    bullet_boat->getMotionState()->getWorldTransform(boat_transform);
    btVector3 position = boat_transform.getOrigin();
    btQuaternion attitude = boat_transform.getRotation();
    btScalar roll, pitch, yaw;
    boat_transform.getBasis().getEulerZYX(roll, yaw, pitch);
    dynamics.position.keys.x = position.z();
    dynamics.position.keys.y = position.x();
    dynamics.position.keys.z = position.y();
    dynamics.attitude.keys.x = roll;
    dynamics.attitude.keys.y = yaw;
    dynamics.attitude.keys.z = pitch;

    btVector3 linear_velocity = bullet_boat->getLinearVelocity();
    btVector3 angular_velocity = bullet_boat->getAngularVelocity();
    
    dynamics.A(0) = (linear_velocity.z() - dynamics.V(0)) / dynamics.time_step_size / 1000.0;
    dynamics.A(1) = (linear_velocity.x() - dynamics.V(1)) / dynamics.time_step_size / 1000.0;
    dynamics.A(2) = (linear_velocity.y() - dynamics.V(2)) / dynamics.time_step_size / 1000.0;
    dynamics.A(3) = (angular_velocity.x() - dynamics.V(3))/ dynamics.time_step_size / 1000.0;
    dynamics.A(4) = (angular_velocity.z() - dynamics.V(4))/ dynamics.time_step_size / 1000.0;
    dynamics.A(5) = (angular_velocity.y() - dynamics.V(5))/ dynamics.time_step_size / 1000.0;
    
    dynamics.V(0) = linear_velocity.z();
    dynamics.V(1) = linear_velocity.x();
    dynamics.V(2) = linear_velocity.y();
    dynamics.V(3) = angular_velocity.x();
    dynamics.V(4) = angular_velocity.z();
    dynamics.V(5) = angular_velocity.y();
...
std::pair<ASVLite::Geometry::Coordinates3D, ASVLite::Geometry::Coordinates3D> ASVLite::get_wave_glider_thrust(const Asv& wave_glider, double rudder_angle, double tuning_factor_thrust) {
    ...
    return_value.second.keys.x = tuning_factor_thrust * thrust;
    ...
    const double F_L_rudder = 0.5 * Constants::SEA_WATER_DENSITY * C_L_2 * A * V_surge * V_surge;
