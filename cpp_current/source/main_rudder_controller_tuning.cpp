...
int main() {
    ...
    ASVLite::RudderController rudder_controller(asv_spec, {1,1,1});
    rudder_controller.tune_controller_gradient_descent(0, 5, 0.25);
    ...
    const ASVLite::SeaSurface sea_surface {wave_ht, wave_dp, wave_rand_seed, count_component_waves};
    ...
    while(asv.get_time() < simulation_duration && i < waypoints.size()) {
        const double rudder_angle = rudder_controller.get_rudder_angle(asv, waypoints[i]);
