...
struct Asv
{
  ...
  bool halt_surge_and_sway;  //!< Set variable to true to keep the ASV stationary.
...
static void set_net_force(struct Asv* asv)
{
  ...
  if(asv->halt_surge_and_sway == true)
  {
...
void asv_set_surge_sway_halt(struct Asv* asv, bool status)
{
  ...
  {
    clear_error_msg(&asv->error_msg);
    asv->halt_surge_and_sway = status;
  }
  else
  {
    set_error_msg(&asv->error_msg, error_null_pointer);
